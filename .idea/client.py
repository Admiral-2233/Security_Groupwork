...
async def run(user_id: str, server_url: str):
    ...
    async with ws_connect(server_url) as ws:
        # HELLO（首帧可不签名）  :contentReference[oaicite:25]{index=25}
        ...
        async def rx():
            ...
            while True:
                ...
                typ = env["type"]
                if typ == "USER_DELIVER":
                    # DM：解密并验签（content_sig: ciphertext|from|to|ts）
                    ...
                    # 群聊：用频道私钥解密，并按 public 规则验 content_sig（ciphertext|from|ts）
        ...
        async def tx():
            ...
            while True:
                ...
                if line == "/list":
                    ...
                    await ws.send(json.dumps(env, separators=(",", ":")))
                    # 分片并对每段做 RSA-OAEP（与消息一致）  :contentReference[oaicite:26]{index=26}
                    pk_b64 = get_pubkey(DB, dst)
                    if not pk_b64:
