...
async def route_to_user(frame: dict):
    ...
    # Return to source as needed; simplified to broadcast to all servers
...
async def handle_server_frame(ws, env):
    ...
    if typ == "SERVER_ANNOUNCE":
        ...
        # Update channel status (member wraps version)
        if not env_ok_by_server(env): return
        # Just broadcast; this demo doesn't persist wraps on server (clients receive shares)
        await broadcast_servers(env)

    elif typ == "PUBLIC_CHANNEL_KEY_SHARE":
        # Broadcast to all servers; each server delivers shares to local users
        if not env_ok_by_server(env): return
...
async def handle_user_frame(ws, env):
    ...
    if typ == "USER_HELLO":
        ...
        # Register locally and record in directory
        local_users[uid] = ws
        ...
        # Let them join public channel and distribute key share (only log exceptions)
        try:
        ...
        # Finally send current online user list to new user (to avoid GUI starting blank)
        try:
    ...
    # Other branches unchanged...
        ...
        # MUST: server doesn't decrypt; verify content_sig and (optional) envelope.sig
        ...
        # envelope.sig (user link) optional per §7; can enforce directory pubkey verification here if needed.
        # Routing
        ...
        # MUST: content_sig = SHA256(ciphertext||from||ts) (no 'to')  :contentReference[oaicite:24]{index=24}
        spub = import_pub_b64url(env["payload"]["sender_pub"])
        ...
        # DM file (same routing as MSG_DIRECT)
    ...
    try:
        ...
        async for msg in ws:
            try:
                ...
                # Print full stack trace but don't kill connection; continue to next frame
        ...
        # Cleanup (maintain original logic)
...
async def add_to_public_channel_and_keyshare(uid: str):
    ...
    # Broadcast channel update (optional, mainly for syncing member changes with other servers)
    try:
    ...
    # Generate and distribute key share (wrap channel private key with member's public key using OAEP)
    ...
    try:
        ...
        # Local user: deliver directly to user session (avoids "can't receive my own share" issue)
        if uid in local_users:
        ...
        # Also broadcast to other servers (their local users will be forwarded in handle_server_frame)
